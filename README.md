...
- **Order Confirmation Emails:** Sending order confirmation emails to customers after a successful purchase. The email has details the order ID, order date,    
  purchased items (name, price, quantity, total price), and total amount of the order.
...
## Additional Features
- **User Authentication:** Basic user authentication is implemented to ensure only registered users can place order (JWT Token).
